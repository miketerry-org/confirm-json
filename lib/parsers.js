...
function authRoleParser(value) {
  return enumParser("authRole")(value);
}
 ...
 */
const enumParser = (name) => (value) => {
  // first find the enumeration
  let item = enums.find(name);
  if (!item) {
    throw new Error(`"${name}" is not a defined enumeration`);
  }
  ...
  for (const enumValue of item.values) {
